...
public class TemperatureGraph extends AppCompatActivity {
    ...
    private ViewGroup layoutTemperatureGraphView;
    ...
    void DrawGraph(){
        try {
            ...
            temperature    = new float[jsonArray.length()];
            //Toast.makeText(getApplicationContext(), "test2", Toast.LENGTH_SHORT).show();
            //GRAPH SETTING
            for(int i=0; i<jsonArray.length(); i++){
                timeStamp[i] = jsonArray.getJSONObject(i).getString("time");
                temperature[i] = jsonArray.getJSONObject(i).getInt("temperature");
            }
    ...
    private void setLineGraph() {
        ...
        layoutTemperatureGraphView.addView(new LineGraphView(this, vo));
    }
